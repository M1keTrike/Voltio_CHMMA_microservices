...
func triggerAction(rule rules.AutomationRule) {
    if rule.ActionCapabilityID == 1 {
        topic := rule.ActionDeviceMAC + ".command"
        body := []byte(rule.ActionPayload)
        err := channel.Publish(
            "amq.topic",
            topic,
            false,
        ...
        if err != nil {
            log.Printf("[RabbitMQ] Error publicando acción: %v", err)
        } else {
            log.Printf("[RabbitMQ] Acción disparada: %s -> %s", topic, string(body))
        }
    } else {
        log.Printf("[RabbitMQ] Acción ignorada: action_capability_id != 1 (valor: %v)", rule.ActionCapabilityID)
    }
}
